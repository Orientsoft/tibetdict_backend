...
def judge_word(content: str):
    header_re = "[^།་\d\]\u4e00-\u9fa5a-zA-Z]"
    footer_re = "[འི|འུ|འོ|ས|ར]?[་|།]$"
    if re.match(f"{header_re}.*{footer_re}", content):
        return True
    else:
        return False
...
if __name__ == '__main__':
    ...
    para = 'a་བཅོམ་ལྡན་འདས་ ཀྱི་  ་བཅོམ་ལྡན་འདསར་  ་བཅོམ་ལྡན་འདསརa'
    a = "གི།ཁོ།"
    b = "ངོ་ཚ་བ།ཁ་"
    # print(re.search('n',para))
    # [' ', ']', u']', u'་', u'།']
    header_re = "[^།་\d\]\u4e00-\u9fa5a-zA-Z]"
    footer_re = "[འི|འུ|འོ|ས|ར]?[་|།]$"
    print(re.match(f"{header_re}.*{footer_re}", a))

    # p = re.compile(f"{header_re}བཅོམ་ལྡན་འདས{footer_re}")
